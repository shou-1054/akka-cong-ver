...
final case class WatermarkRequestStrategy(highWatermark: Int, lowWatermark: Int) extends RequestStrategy {
  ...
  def requestDemand(remainingRequested: Int): Int = {
    if (remainingRequested < lowWatermark) {
      request = highWatermark - remainingRequested
      overFlowCounter += 1
    }
    else request = 0

    print("overFlowCounter:")
    println(overFlowCounter)

    return request
  }
}

object TahoeRequestStrategy {
  /**
  * Create [[TahoeRequestStrategy]].
  */
  //def apply(capacity: Int): TahoeRequestStrategy = new TahoeRequestStrategy(capacity)
}

final case class TahoeRequestStrategy(capacity: Int) extends RequestStrategy {
  ...
  */
  ...
  def overFlowAvoidance() {
    cwnd += 1
  }

  def searchLimit(remainingRequested: Int) {
    if (remainingRequested < capacity)
      slowStart()
    else {
      limit = cwnd
      ssthresh = limit / 2
...
object RenoRequestStrategy {
  /**
  * Create [[RenoRequestStrategy]].
  */
  //def apply(limit: Int): RenoRequestStrategy = new RenoRequestStrategy(limit)
}
...
object ConjectureRequestStrategy {
  /**
   * Create [[ConjectureRequestStrategy]] with `ssthresh` as half of
   * the specified `limit`.
   */
  def apply(limit: Int): ConjectureRequestStrategy = new ConjectureRequestStrategy(limit)
}

/**
 * The publisher will send the element predicting the number of subscriber buffers.
 */
final case class ConjectureRequestStrategy(var limit: Int, var ssthresh: Int) extends RequestStrategy {
  require(ssthresh >= 0, "ssthresh must be >= 0")
  require(limit >= ssthresh, "limit must be >= ssthresh")
  ...
  def requestDemand(remainingRequested: Int): Int = {
    ...
    if (limit - 1 > remainingRequested && stop == false && free > 1048576) {
      request = (math.pow(2, n)).toInt
      if (remainingRequested + request > limit) request = limit - remainingRequested
      n += 1
    } else {
      stop = true
      request = 0
      ...
      if (remainingRequested == ssthresh) {
        stop = false
        ssthresh += limit / 6
        if (ssthresh >= limit) ssthresh = limit / 2
      }
    }

    // Print number of publisher sent and Subscriber's buffer.
    //print("Publisher send ")
    //println(request)
    //print("Subscriber's buffer : ")
    //println(remainingRequested)

    //Print memory information.
    //println("total => " + total / 1024 / 1024 + "MB");
    //println("free  => " + free / 1024 / 1024 + "MB");
    //println("used  => " + (total - free) / 1024 / 1024 + "MB");
    //println("max   => " + max / 1024 / 1024 + "MB");
    return request
  }
}
